...
- Automated gates (`nub run check`, CI) MUST stay in-repo. `nub run check:tokens` reads
...
| Effect-async           | `lens` jsPlugin: `lens/no-async-function`, `lens/no-await-expression`, `lens/no-new-promise` (tests exempt via frozen override); retired foldstryx allowlist still frozen by `scripts/check-async-allowlist.mjs` | `async` / `await` / `new Promise` in library code                                                                                                                                                                                                                                                                                                           |
| Waiver ratchet         | `scripts/check-waiver-allowlist.mjs` + hk pre-commit / pre-push + `nub run check`                                                                                                                                | Disable comments, oxlint/fallow/changeset exceptions, and `AWAIT_ALLOWLIST` cannot grow without updating the frozen baseline                                                                                                                                                                                                                                |
...
| Token fidelity         | `nub run check:tokens`                                                                                                                                                                                           | In-repo NOTICE pin + lifted token names/values (no `../astryx`)                                                                                                                                                                                                                                                                                             |
| Demo smoke             | `nub run check:demo`                                                                                                                                                                                             | Fresh Vite preview on an ephemeral port (no reuse of `nub run dev`); needs `google-chrome` or `CHROME_PATH`. CI sets `CHROME_NO_SANDBOX=1`                                                                                                                                                                                                                  |
| Packed boundary        | `nub run check:packed`                                                                                                                                                                                           | Packs the five external-consumer packages, installs the tarballs into a throwaway Vite consumer (no workspace links), builds, asserts the emitted CSS contains StyleX rules, and drives Chrome to verify the docs shell, a route transition, and font loading from package assets. Needs `google-chrome`/`CHROME_PATH` and network for the consumer install |
...
Waivers are frozen. `nub run check:waivers` (hk pre-commit / pre-push, `nub run check`, and CI) fails if a new `oxlint-disable` / `@ts-expect-error` / prettier-ignore appears, if oxlint/fallow/changeset exceptions grow, or if `AWAIT_ALLOWLIST` changes. Shrink or growth both require editing `scripts/check-waiver-allowlist.mjs` in the same change. hk is local hooks only; CI runs `nub run check` plus the dedicated Effect Lens job (`nub run check:effect-lens`).
...
- **Visual slices**: browser verification at `http://localhost:5173/` plus the
  `nub run check:demo` smoke probe.
